...
pub fn is_moonflare_workspace() -> bool {
    Path::new(".moon/workspace.yml").exists() || Path::new("package.json").exists()
}
...
pub fn add_wasm_dependency_to_project(project_path: &Path) -> Result<()> {
        ...
        && let Some(build_mapping) = build_task.as_mapping_mut()
    {
        ...
        if !has_wasm_dep {
            ...
            build_mapping.insert(Value::String("deps".to_string()), Value::Sequence(deps));
        }
        ...
        let has_wasm_input = inputs
            .iter()
            .any(|input| input.as_str() == Some(wasm_input));

        if !has_wasm_input {
            inputs.push(Value::String(wasm_input.to_string()));
            build_mapping.insert(Value::String("inputs".to_string()), Value::Sequence(inputs));
        }
...
pub fn add_crate_build_dependency_to_shared_wasm(crate_name: &str) -> Result<()> {
        ...
        && let Some(gather_mapping) = gather_task.as_mapping_mut()
    {
        ...
        let has_crate_dep = deps.iter().any(|dep| dep.as_str() == Some(&crate_target));

        if !has_crate_dep {
            // Add crate:build to deps
            deps.push(Value::String(crate_target));
            gather_mapping.insert(Value::String("deps".to_string()), Value::Sequence(deps));
        }
    }
