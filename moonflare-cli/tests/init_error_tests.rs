...
fn test_init_fails_with_invalid_name_characters() -> anyhow::Result<()> {
    ...
    for name in invalid_names {
        ...
        if name.is_empty() {
            ...
            log(&format!("Empty name command success: {}", output.status.success()));
            ...
            log(&format!("Command output for '{}': success={}", name, output.status.success()));
...
fn test_init_fails_when_directory_already_exists() -> anyhow::Result<()> {
    ...
    log(&format!("stderr: {}", String::from_utf8_lossy(&output.stderr)));
    
    // Verify original file still exists if command failed
    if !output.status.success() {
        assert!(existing_dir.join("existing-file.txt").exists(), "Original file should still exist");
    }
    ...
    for path in restricted_paths {
        ...
        if !output.status.success() {
            ...
            if !has_descriptive_error {
                log(&format!("Warning: Error message may not be descriptive enough: {}", stderr));
            }
        }
    }
...
fn test_init_validates_workspace_name_format() -> anyhow::Result<()> {
    ...
    for name in edge_case_names {
        log(&format!("Testing edge case name (len={}): '{}'", name.len(), 
        ...
        log(&format!("Result for '{}' (len={}): success={}", 
...
fn test_init_handles_filesystem_errors() -> anyhow::Result<()> {
    ...
    if !output.status.success() {
        ...
        assert_eq!(content, "existing file content", "Original file should be unchanged");
    }
...
fn test_init_with_minimal_disk_space() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Workspace should not exceed 10MB, got {} bytes", total_size
    );
