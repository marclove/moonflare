...
fn test_init_creates_workspace_structure() -> anyhow::Result<()> {
    ...
    assert!(project_path.join("apps").is_dir(), "apps directory should exist");
    assert!(project_path.join("sites").is_dir(), "sites directory should exist");
    assert!(project_path.join("workers").is_dir(), "workers directory should exist");
    assert!(project_path.join("crates").is_dir(), "crates directory should exist");
    
    // Verify workspace configuration files exist
    assert!(project_path.join(".moon").is_dir(), ".moon directory should exist");
    assert!(project_path.join("package.json").is_file(), "package.json should exist");
    assert!(project_path.join("pnpm-workspace.yaml").is_file(), "pnpm-workspace.yaml should exist");
    assert!(project_path.join("README.md").is_file(), "README.md should exist");
...
fn test_init_creates_valid_package_json() -> anyhow::Result<()> {
    ...
    assert_eq!(package_json["name"], "my-workspace", "Package name should match workspace name");
    assert!(package_json["private"].as_bool().unwrap_or(false), "Package should be private");
    assert!(package_json["scripts"].is_object(), "Should have scripts section");
    assert!(package_json["devDependencies"].is_object(), "Should have devDependencies");
...
fn test_init_creates_valid_moon_config() -> anyhow::Result<()> {
    ...
    let workspace_yml_path = workspace.path().join("test-workspace").join(".moon").join("workspace.yml");
    assert!(workspace_yml_path.exists(), ".moon/workspace.yml should exist");
...
fn test_init_creates_pnpm_workspace_config() -> anyhow::Result<()> {
    ...
    let pnpm_workspace_path = workspace.path().join("test-workspace").join("pnpm-workspace.yaml");
    assert!(pnpm_workspace_path.exists(), "pnpm-workspace.yaml should exist");
...
fn test_init_with_different_names() -> anyhow::Result<()> {
    ...
    for name in test_names {
        ...
        assert!(project_path.is_dir(), "Project directory should exist for name: {}", name);
        
        // Verify package.json has correct name
        let package_json_path = project_path.join("package.json");
        let content = fs::read_to_string(&package_json_path)?;
        let package_json: serde_json::Value = serde_json::from_str(&content)?;
        assert_eq!(package_json["name"], name, "Package name should match for: {}", name);
    }
...
fn test_init_workspace_can_add_projects_immediately() -> anyhow::Result<()> {
    ...
    assert!(project_path.join("sites").join("test-site").is_dir(), "Astro site should be created");
    assert!(project_path.join("apps").join("test-app").is_dir(), "React app should be created");
    assert!(project_path.join("crates").join("test-crate").is_dir(), "Crate should be created");
