...
fn test_init_dot_in_empty_directory() -> anyhow::Result<()> {
    ...
    assert!(subdir.join("sites").is_dir(), "sites directory should exist");
    assert!(subdir.join("workers").is_dir(), "workers directory should exist");
    assert!(subdir.join("crates").is_dir(), "crates directory should exist");
    assert!(subdir.join(".moon").is_dir(), ".moon directory should exist");
    assert!(subdir.join("package.json").is_file(), "package.json should exist");
...
fn test_init_dot_in_non_empty_directory_fails() -> anyhow::Result<()> {
    ...
    assert!(subdir.join("existing-file.txt").exists(), "Original file should still exist");
    assert!(subdir.join("README.md").exists(), "Original README should still exist");
    assert!(!subdir.join(".moon").exists(), ".moon directory should not be created");
...
fn test_init_dot_with_force_in_non_empty_directory() -> anyhow::Result<()> {
    ...
    assert!(subdir.join("existing.txt").exists(), "Original file should still exist");
    assert!(subdir.join("another.js").exists(), "Original JS file should still exist");
...
fn test_init_dot_with_invalid_directory_name() -> anyhow::Result<()> {
    ...
    assert!(!output.status.success(), "Init should fail with invalid directory name");
