...
fn test_rename_react_project_updates_directory_and_wrangler_jsonc() -> anyhow::Result<()> {
    ...
    assert!(
        old_path.is_dir(),
        "Original frontend directory should exist"
    );
    ...
    assert!(
        original_content.contains(r#""name": "frontend""#),
        "Original config should have frontend name"
    );
    ...
    assert!(
        wrangler_config_new.is_file(),
        "wrangler.jsonc should exist in new location"
    );

    let updated_content = fs::read_to_string(&wrangler_config_new)?;
    assert!(
        updated_content.contains(r#""name": "web-app""#),
        "Updated config should have web-app name"
    );
    assert!(
        !updated_content.contains(r#""name": "frontend""#),
        "Updated config should not have old name"
    );

    // Verify other files were preserved
    assert!(
        new_path.join("package.json").is_file(),
        "package.json should be preserved"
    );
    assert!(
        new_path.join("src").is_dir(),
        "src directory should be preserved"
    );
...
fn test_rename_durable_object_updates_directory_and_wrangler_toml() -> anyhow::Result<()> {
    ...
    let original_content = fs::read_to_string(&wrangler_config)?;
    assert!(
        original_content.contains(r#"name = "api""#),
        "Original config should have api name"
    );
    ...
    assert!(
        wrangler_config_new.is_file(),
        "wrangler.toml should exist in new location"
    );

    let updated_content = fs::read_to_string(&wrangler_config_new)?;
    assert!(
        updated_content.contains(r#"name = "backend""#),
        "Updated config should have backend name"
    );
    assert!(
        !updated_content.contains(r#"name = "api""#),
        "Updated config should not have old name"
    );

    // Verify other content was preserved
    assert!(
        updated_content.contains("[[durable_objects.bindings]]"),
        "Durable object bindings should be preserved"
    );
...
fn test_rename_astro_project_updates_directory_and_wrangler_jsonc() -> anyhow::Result<()> {
    ...
    let updated_content = fs::read_to_string(&wrangler_config_new)?;
    assert!(
        updated_content.contains(r#""name": "marketing""#),
        "Updated config should have marketing name"
    );
    assert!(
        !updated_content.contains(r#""name": "site""#),
        "Updated config should not have old name"
    );
...
fn test_rename_crate_project_updates_directory_only() -> anyhow::Result<()> {
    ...
    assert!(
        old_path.join("Cargo.toml").is_file(),
    ...
    assert!(
        new_path.join("Cargo.toml").is_file(),
    ...
    assert!(
        new_path.join("src").join("lib.rs").is_file(),
        "lib.rs should be preserved"
    );

    // Verify no wrangler files (crates don't have them)
    assert!(
        !new_path.join("wrangler.toml").exists(),
        "Crates should not have wrangler.toml"
    );
    assert!(
        !new_path.join("wrangler.json").exists(),
        "Crates should not have wrangler.json"
    );
    assert!(
        !new_path.join("wrangler.jsonc").exists(),
        "Crates should not have wrangler.jsonc"
    );
...
fn test_rename_allows_same_name_across_different_project_types() -> anyhow::Result<()> {
    ...
    assert!(
        project_path.join("apps").join("core").is_dir(),
        "React core should exist"
    );
    assert!(
        project_path.join("crates").join("core").is_dir(),
        "Crate core should exist"
    );
...
fn test_rename_preserves_wrangler_json_format() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Original file should have block comments"
    );
    assert!(
        original_content.contains("*/"),
        "Original file should have block comments"
    ...
    assert!(
        ...
        "Renamed file should preserve block comments"
    );
    assert!(
        updated_content.contains("*/"),
        "Renamed file should preserve block comments"
...
fn test_rename_all_project_types_comprehensive() -> anyhow::Result<()> {
    ...
    assert!(
        project_path.join("apps").join("web-client").is_dir(),
        "React renamed"
    );
    assert!(
        project_path.join("sites").join("landing").is_dir(),
        "Astro renamed"
    );
    assert!(
        project_path.join("workers").join("state-manager").is_dir(),
        "DO renamed"
    );
    assert!(
        project_path.join("crates").join("wasm-utils").is_dir(),
        "Crate renamed"
    );

    // Verify old directories are gone
    assert!(
        !project_path.join("apps").join("react-app").exists(),
        "Old React gone"
    );
    assert!(
        !project_path.join("sites").join("astro-site").exists(),
        "Old Astro gone"
    );
    assert!(
        !project_path.join("workers").join("durable-obj").exists(),
        "Old DO gone"
    );
    assert!(
        !project_path.join("crates").join("rust-crate").exists(),
        "Old crate gone"
    );
    ...
    assert!(
        web_client_config.contains(r#""name": "web-client""#),
        "React wrangler updated"
    );
    ...
    assert!(
        landing_config.contains(r#""name": "landing""#),
        "Astro wrangler updated"
    );
    ...
    assert!(
        state_manager_config.contains(r#"name = "state-manager""#),
        "DO wrangler updated"
    );
