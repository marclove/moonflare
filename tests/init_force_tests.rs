...
fn test_force_flag_with_regular_project_names() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Init should fail in non-empty directory without force"
    );
    ...
    assert!(
        project_dir.join("apps").is_dir(),
        "apps directory should exist"
    );
    assert!(
        project_dir.join("existing.txt").exists(),
        "Original file should exist"
    );
    assert!(
        project_dir.join("README.md").exists(),
        "Original README should exist"
    );
    assert!(
...
fn test_force_flag_with_hidden_files() -> anyhow::Result<()> {
    ...
    assert!(
        !output.status.success(),
        "Should fail without --force due to visible file"
    );
    ...
    assert!(
        subdir.join(".hidden").exists(),
        "Hidden file should be preserved"
    );
    assert!(
        subdir.join(".gitignore").exists(),
        "Gitignore should be preserved"
    );
    assert!(
        subdir.join("visible.txt").exists(),
        "Visible file should be preserved"
    );
...
fn test_force_flag_with_subdirectories() -> anyhow::Result<()> {
    ...
    assert!(
        subdir.join("existing-subdir").is_dir(),
        "Existing subdirectory should be preserved"
    );
    assert!(
        nested_dir.join("nested-file.txt").exists(),
        "Nested file should be preserved"
    );
    assert!(
        subdir.join("root-file.txt").exists(),
        "Root file should be preserved"
    );

    // Verify workspace was created
    assert!(
        subdir.join("apps").is_dir(),
        "apps directory should be created"
    );
    assert!(
        subdir.join("package.json").exists(),
        "package.json should be created"
    );
...
fn test_init_dot_with_path_flag() -> anyhow::Result<()> {
    ...
    assert!(
        workspace_dir.join("apps").is_dir(),
        "Workspace should be created with proper structure"
    );
    assert!(
    ...
    assert_eq!(
        package_json["name"], "valid-workspace-name",
